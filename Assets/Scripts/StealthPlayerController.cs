...
public class StealthPlayerController : Character {
    ...
    public bool canShoot = true; // TAREFA 1: Habilidade de tiro (habilitada por padrão)

    [Header("TAREFA 1: Habilidade de Tiro")]
    [Tooltip("Prefab do projétil do jogador")]
    public GameObject playerBulletPrefab;
    
    [Tooltip("Custo de energia por tiro")]
    public float shootEnergyCost = 5f;
    
    [Tooltip("Cooldown entre tiros em segundos")]
    public float shootCooldown = 0.3f;
    
    [Tooltip("Offset vertical do ponto de spawn do projétil")]
    public float bulletSpawnHeight = 0.3f;
    
    [Tooltip("Offset frontal do ponto de spawn do projétil")]
    public float bulletSpawnForward = 0.5f;
    
    // Timer interno para cooldown do tiro
    private float shootCooldownTimer = 0f;

    [Header("TAREFA 2: Habilidade de Hover")]
    [Tooltip("Referência ao componente HoverAbility (opcional, buscado automaticamente)")]
    ...
    void Start () {
        ...
        // TAREFA 2: Busca automaticamente o HoverAbility se não estiver configurado
        if (hoverAbility == null)
        {
            hoverAbility = GetComponent<HoverAbility>();
        }
	}
    ...
    {
        energy -= val;
        // VALIDAÇÃO: Garante que energia nunca fique negativa
    ...
    void Update() {
        ...
        // Verifica input de tiro (Fire1 = botão esquerdo do mouse ou botão de gamepad)
        if (canShoot && Input.GetButtonDown("Fire1") && (state == States.idle || state == States.moving))
        {
            TryShoot();
        }
        ...
        if (enableEnergyDrain)
        {
            ...
            // TAREFA 2: Aplica multiplicador de energia do Hover
            if (hoverAbility != null && hoverAbility.IsHovering)
            {
                energyDrain = energyDrain * hoverAbility.GetEnergyMultiplier();
            }
    ...
    /// <summary>
    /// TAREFA 1: Tenta disparar um projétil.
    /// Verifica energia disponível, cooldown e cria o projétil se possível.
    /// TAREFA 2: Adiciona feedback visual/sonoro e camera shake.
    /// </summary>
    void TryShoot()
    {
        ...
        // Calcula custo real considerando Assist Mode
        float realCost = GetAdjustedEnergyCost(shootEnergyCost);
        
        // Verifica se tem energia suficiente
        if (energy < realCost)
        {
            // TAREFA 2: Feedback visual para energia insuficiente
            OnInsufficientEnergy("Tiro");
            return;
        }
        
        // TAREFA 2: Verificação defensiva do prefab
        if (playerBulletPrefab == null)
        {
            // Tenta carregar do Resources como fallback
            playerBulletPrefab = Resources.Load<GameObject>("PlayerBullet");
            
            if (playerBulletPrefab == null)
            {
                Debug.LogWarning("[Shoot] playerBulletPrefab não configurado. Tiro desabilitado para esta sessão.");
                canShoot = false; // Desabilita para evitar spam de warnings
                return;
            }
        }
        
        // Consome energia
        SpendEnergy(realCost);
        
        // Inicia cooldown
        shootCooldownTimer = shootCooldown;
        
        // Calcula posição de spawn do projétil
        Vector3 spawnPosition = transform.position 
            + Vector3.up * bulletSpawnHeight 
            + transform.forward * bulletSpawnForward;
        
        // Cria o projétil
        GameObject bullet = Instantiate(playerBulletPrefab, spawnPosition, transform.rotation);
        
        // Configura a direção do projétil
        bullet.transform.forward = transform.forward;
        
        // Cria efeito visual de disparo se disponível
        if (EffectsManager.getInstance() != null && EffectsManager.getInstance().shootEffect != null)
        {
            GameObject shootEffect = Instantiate(EffectsManager.getInstance().shootEffect);
            shootEffect.transform.position = spawnPosition;
            shootEffect.transform.forward = transform.forward;
        }
        
        // TAREFA 2: Camera shake ao atirar (juice)
    ...
    #region TAREFA 2 & 3: Métodos Auxiliares e Feedback
    
    /// <summary>
    /// TAREFA 2: Feedback quando energia é insuficiente para uma habilidade.
    /// Centraliza a lógica de feedback para manutenção.
    /// </summary>
    /// <param name="abilityName">Nome da habilidade para log</param>
    void OnInsufficientEnergy(string abilityName)
    {
        // Flash vermelho na barra de energia
        if (energyBar != null)
        {
            energyBar.Flash(Color.red, 0.3f);
        }
    ...
    /// <summary>
    /// TAREFA 6: Retorna custo de energia ajustado pelo Assist Mode.
    /// </summary>
    float GetAdjustedEnergyCost(float baseCost)
    {
        if (DebugOverlay.Instance != null)
        {
            return baseCost * DebugOverlay.Instance.GetAssistModeMultiplier();
        }
        return baseCost;
    }
    
    /// <summary>
    /// TAREFA 3: Verifica se o jogador pode usar habilidades.
    /// Método auxiliar para validação centralizada.
    /// </summary>
    public bool CanUseAbilities()
    {
        // Verifica estado do jogo
        if (GameLogic.instance == null) return false;
        if (GameLogic.instance.gameState != GameLogic.GameStates.gameplay) return false;
        
        // Verifica estado do jogador
        if (state != States.idle && state != States.moving) return false;
        
        return true;
    }
    
    /// <summary>
    /// TAREFA 3: Verifica se tem energia suficiente para uma ação.
    /// </summary>
    public bool HasEnergy(float amount)
    {
        float adjustedAmount = GetAdjustedEnergyCost(amount);
        return energy >= adjustedAmount;
    }
